...
function checkStatus(){
    if (localStorage.getItem('darkMode')==="true"){
        ...
        link.href = './Scss/togglemode.css?ts=<?=time()?>';                   //since it's true we load the dark theme CSS
    }else{
...
function dark_mode_status(){                                //This function gets called every time the checkbox is clicked
    if (localStorage.getItem('darkMode')==="true"){     //if darkMode was active and this function is called it means the user now wants light
        ...
        link.href = './Scss/togglemode.css?ts=<?=time()?>';
    }
}